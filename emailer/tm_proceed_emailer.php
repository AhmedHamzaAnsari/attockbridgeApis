...
if ($count_eng > 0) {
    while ($row = mysqli_fetch_array($result_eng)) {
        ...
        $msg_subject_tm = $dealer_name_global . ' order processed successfully';
        $msg_body_tm = '<p>Hi ' . $tm_name . ',</p><p>Order of ' . $dealer_name_global . ' has been processed successfully.</p>';

        // Compose email message for dealer
        $msg_subject_dealer = $dealer_name_global . ' order processed successfully';
        $msg_body_dealer = '<p>Hi ' . $dealer_name . ',</p><p>Your order of ' . $dealer_name_global . ' has been processed successfully.</p>';
...
if ($count_App_order > 0) {
    while ($row = mysqli_fetch_array($result_App_order)) {
        ...
        $msg_subject = $dealer_name_global . ' order processed successfully ';
        $msg_body = '<p>Hi ' . $name . '</h3><p>Order of ' . $dealer_name_global . ' processed successfully. Please Verify and Forward.</p>';
