...
define('DB_PASSWORD', '');
define('DB_DATABASE', 'attockbridge');
...
$eng_query = "SELECT dl.name as dealer_name, us.name as tm_name, us.login as tm_email, dl.email as dealer_email,om.web_order
...
if ($count_eng > 0) {
    while ($row = mysqli_fetch_array($result_eng)) {
        ...
        $is_web = $row["web_order"];

        // Compose email message
        $msg_subject_tm = $dealer_name_global . ' order puched successfully';
        $msg_body_tm = '<p>Hi ' . $tm_name . ',</p><p>Order of ' . $dealer_name_global . ' has been Pushed successfully.</p>';

        // Compose email message for dealer
        $msg_subject_dealer = $dealer_name_global . ' order pushed successfully';
        $msg_body_dealer = '<p>Hi ' . $dealer_name . ',</p><p>Your order of ' . $dealer_name_global . ' has been Pushed successfully.</p>';
...
if ($count_App_order > 0) {
    while ($row = mysqli_fetch_array($result_App_order)) {
        ...
        $msg_subject = $dealer_name_global . ' order pushed successfully ';
        $msg_body = '<p>Hi ' . $name . '</h3><p>Order of ' . $dealer_name_global . ' has been Pushed successfully.</p>';
...
if($is_web!=1){
    ...
    if ($count_App_order > 0) {
        while ($row = mysqli_fetch_array($result_forwarded_order)) {
            ...
            echo smtp_mailer('sohaib.bokhari@hascol.com', date('Y-m-d H:i:s'), $dealer_name_global, 'Sohaib', $msg_subject, $msg_body);
        }
    }
...
function smtp_mailer($tm_email, $time, $dealers_name, $tm_name, $msg_subject, $msg_body)
{
    ...
    $mail->Port = 587;
    $mail->Username = "hascol.alertinfo@gmail.com";
    $mail->Password = "lbbsvqnavflugosn";
    $mail->SetFrom("hascol.alertinfo@gmail.com");
