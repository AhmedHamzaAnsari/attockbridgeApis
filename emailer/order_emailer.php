...
if ($count_eng > 0) {
    while ($row = mysqli_fetch_array($result_eng)) {
        ...
        echo smtp_mailer('sohaib.bokhari@hascol.com', date('Y-m-d H:i:s'), $dealer_name_global, 'Sohaib');
    }
}
...
function smtp_mailer($tm_email, $time, $dealers_name, $tm_name)
{
    ...
    $mail->Body = '<h1>Hascol.</h1> <h3>Hi ' . $tm_name . '</h3><p> Please Verify and proceed the of Order of '.$dealers_name.'</p>';				//An HTML or plain text message body
