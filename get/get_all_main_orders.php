...
$from = $_GET["from"];
$to = $_GET["to"];
$rettype = isset($_GET["rettype"]) ? $_GET["rettype"] : '';

// $ret = '';

// if($rettype!='' && $pre=='ASM' && $pre=='TM'){
//     $ret = 'and dl.rettype='.$rettype.'';
// }else{
//     $ret ='';
// }
...
if (!empty($pass)) {
    ...
    WHERE $condition AND om.created_at>='$from' AND om.created_at<='$to' and dl.rettype='$rettype'
    ORDER BY om.id DESC;
        ";
