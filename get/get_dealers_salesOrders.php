...
if ($pass != '') {
    if ($pass == $access_key) {
        $sql_query1 = "SELECT 
        CONCAT(si.order_date, ' ', si.datetime) AS order_datetime,
        si.*, 
        ...
        CASE 
            WHEN dl.rettype_desc = 'COCO site                     ' THEN si.quantity * dp.nozel_price
            ELSE si.quantity * dp.indent_price
        END AS total_amount,
        dl.rettype_desc,
        si.quantity,
        dp.indent_price,
        ...
        JOIN dealers_products AS dp ON dp.dealer_id = dl.id and dp.name=pp.name where dl.id='$id' and si.created_at >='2024-10-01' and si.status!=3 group by si.order_no
        ...
        while ($user = $result1->fetch_assoc()) {
            ...
            if (isset($thread['created_at'])) {
                $thread['created_at'] = $thread['order_datetime'];
            }
        }
